...
$(document).ready(function() {
	blockCategoryTable = undefined;
	tbl_supplier_block = undefined;
	
	console.log("Inside");
	tbl_supplier_block = $('#tbl_supplier_block').DataTable({
			 processing: true,
			 serverSide: true,
			 responsive: true,
		     ajax: {
		    	 url:globalUrl+"/supplier/blocksuppliers",
		         type: 'GET',
...
//					{ "data": "createdDateTime" },
					...
					{ "data": "subCompanyID",
		            	 orderable: false,
		            	 render: function(data,type,row,meta) {
						 return '<button type="button" class="btn btn-primary" style="margin-right: 10px" onclick="viewBlockModle('+data+', '+row.supplierID+' ,\''+row.supName+'\')" class="btn btn-primary" style="margin-right: 10px">Block</button>';
						  },	
...
$('#tbl_supplier_block tbody').on('click', 'td.details-control', function () {
    var tr = $(this).closest('tr');
    var row = tbl_supplier_block.row( tr );
...
function format ( d ) {
    ...
    return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;" class="box box-info animated fadeInDown">'+
        ...
        '<tr>'+	
				...
				'<td><button type="button" class="btn btn-success" onclick = "viewBlockCategories(\''+d.subCompanyID+'\')">View Categories</button></td>'+
		'</tr>'+
...
function viewImage(buttonID, supplierID, name){
	...
	$('#mdl_block_sup').modal('toggle');
}	
	
	
	
var grandTOT = 0;
function viewBlockCategories(supplierID) {
	console.log("blockCategoryTable "+supplierID);
	$('#block_Modal').modal('toggle');
	if (blockCategoryTable == undefined) {
		console.log("Inside Category Suppliers "+supplierID)
		
		blockCategoryTable = $('#block_category_sup').DataTable({
			processing: true,
			serverSide: true,
			responsive: true,
			ajax: {
				url:globalUrl+"/supplier/viewblockcategory/"+supplierID,
				type:'GET',
				headers: { 'Authorization': 'Basic ' + sessionStorage.getItem("GlobalToken") },
			},
			columns: [
		...
		blockCategoryTable.ajax.url(globalUrl+"/supplier/viewblockcategory/"+supplierID).load();
	}
	
	blockCategoryTable.on( 'xhr', function () {
    var json = blockCategoryTable.ajax.json();
...
function viewCat(supplierID) {
			getAsyncData("/supplier/"+supplierID+"/viewblockcategory", function(req) {
		})
}
	  ...
	  confirmButtonText: 'Yes, Block!'
		  ...
		  if (result.value) {
			console.log(data);
				...
				var responseCode = getAsyncDataPUT("/supplier/block-supplier",data, function(res) {
				console.log(res);
				console.log(res.responseJSON);
				  ...
				  if(res.responseJSON.code == "01"){
					...
					tbl_supplier_block.ajax.url(globalUrl + "/supplier/blocksuppliers").load();
					}else{
						Swal.fire(
						'Failed!',
...
var supplierID;
var supplierName;
function viewBlockModle(data, d1, d2){
	$("#ml_supplierBlock_Modal").modal();
...
$('#block_button').on('click', function() {
	let status = 0;
	if ($('#reason_blocksupplier').val().length==0) {
		status+=1;
		$('#reason_blocksupplier').parent().parent().addClass('has-error')
	}		
	var dataObj = {
		"selectSupplierID" : selectSupplierID,
		"reason" : $('#reason_blocksupplier').val(),
	};
	console.log(dataObj);
	Block(dataObj);
