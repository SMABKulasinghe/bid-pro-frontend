 ...
 getAsyncData('/evaluation/trnderid', function(res) {
		...
		console.log(res.responseJSON);
		  ...
		  for (let item of res.responseJSON) {
			  $('#ecc_tenderid').append(new Option(item.bidno+" - "+ item.tendername, item.trnderid))
		}
	});
});
...
function loadTable(tender_ID) {
	...
	tbl_committee = $('#tbl_committee').DataTable({
			 ...
			 responsive: true,
		     ajax: {
		    	 url:globalUrl+"/evaluation/committeeview/tendrID/"+tender_ID,
		         type: 'GET',
		...
		if(!(tender_ID =="")){
			tbl_committee.ajax.url(globalUrl + "/evaluation/committeeview/tendrID/"+tender_ID).load();
		}
	}
}
			...
			Swal.fire({
				  ...
				  if (result.value) {
					  console.log("TT");
						...
						var	 responseCode = getAsyncDataPOST("/evaluation/addevaluationcommittee",committee,confirmedCallBack)
				 }
		});
});
...
function confirmedCallBack(responseCode){
	...
	$("#submit_button").prop("disabled",false);
	   ...
	   if(responseCode.responseJSON.code == "00"){
		   Swal.fire(
				      'Accepted!',
				      'Evaluation Committee Member Creation successful.',
				      'success'
				    )			
	   tbl_committee.ajax.url(globalUrl + "/evaluation/committeeview/tendrID/"+selectedTendderIDfor).load();	
	   }else if(responseCode.responseJSON.code == "01"){
		   Swal.fire({
				type: 'info',
				title: 'Evaluation Committee Member Already Exists',
